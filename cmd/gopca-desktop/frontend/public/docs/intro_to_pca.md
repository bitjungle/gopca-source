...
**PCA method:** Temporal PCA. The tutorial compares an ordinary SVD baseline against L = 5, 10 and 40 to show how the lag window controls which dynamics become visible. L = 40 is needed to resolve the 40-minute flow oscillation as a recognizable sine/cosine pair.
...
- That PCA finds the directions of greatest variance, which need not line up with any variable you care about (coloring by age reveals almost no structure)
...
2. **Spectroscopic Preprocessing:**
   ...
   - **Savitzky-Golay Smoothing and Derivatives**: Fits a low-order polynomial across a sliding window of wavelengths and reads off its value — or its slope, or its curvature — at the center of the window. It can be used alone; combined with SNV or vector normalization it runs after that step, and always before centering.
...
>
> The reason to use Savitzky-Golay rather than subtracting neighboring points is noise. Differentiating amplifies exactly the fast, point-to-point variation that noise consists of, so a derivative taken by plain differencing can be mostly noise. Fitting a smooth polynomial across a window first, and differentiating that, smooths and differentiates in the same step.
>
...
> **One honest caveat.** Keep *every* component and PCR is algebraically identical to OLS, instability and all. The stabilization comes entirely from leaving components out — which is why the choice of how many to keep matters far more here than in exploratory PCA.
